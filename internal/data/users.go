...
*
*/
func (p *password) Set(plaintextPasswords string) error {
	hash, err := bcrypt.GenerateFromPassword([]byte(plaintextPasswords), 12)
	if err != nil {
		return err
	}
	p.plaintext = &plaintextPasswords
	p.hash = hash
...
func ValidatePasswordPlaintext(v *validator.Validator, password string) {
	v.Check(password != "", "password", "must be provide password")
